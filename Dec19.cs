...
namespace AOC._2021
{
    ...
    public class Dec19
    {
        ...
        {
            List<string> items = lines.ToList();
            scanners = new List<Scanner>();
            for (int i = 0; i < items.Count;)
            {
                string line = items[i++];
                string[] splits = SplitOn(line, "--- scanner ", " ---");
                Scanner scanner = new Scanner(int.Parse(splits[1]));

                while (i < items.Count && !string.IsNullOrEmpty(line = items[i++]))
                {
                    splits = SplitOn(line, ",", ",");
                    scanner.Add(int.Parse(splits[0]), int.Parse(splits[1]), int.Parse(splits[2]));
                }
